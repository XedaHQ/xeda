...
class Cocotb(Tool):
    ...
    def __init__(self, settings: 'Cocotb.Settings', cocotb_sim_name: str, run_path: os.PathLike):
    ...
    def vpi_path(self):
        ...
        if self.version_minor >= 5:
            so_path = self.run_tool("cocotb-config",
                                    ["--lib-name-path", "vpi", self.cocotb_name], stdout=True)
        else:
            so_path = self.run_tool("cocotb-config",
