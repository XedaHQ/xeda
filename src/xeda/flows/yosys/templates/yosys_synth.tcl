...
extract_fa
techmap -map {{settings.adder_map|path}}
techmap
opt -purge
{%- endif %}

{%- for map in settings.other_maps %}
techmap -map {{map|path}}
{%- endfor %}

{%- if settings.liberty %}
log -stdout "Prepare mapping FFs to technology"
dfflibmap -prepare -liberty {% if settings.dff_liberty -%} {{settings.dff_liberty|path}} {%- else -%} {{settings.liberty[0]|path}} {%- endif %}
opt -purge
{%- endif %}
...
dfflibmap -liberty {% if settings.dff_liberty -%} {{settings.dff_liberty|path}} {%- else -%} {{settings.liberty[0]|path}} {%- endif %}
opt -purge
{%- endif %}
