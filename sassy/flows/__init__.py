...
class Suite:
    ...
    def get_run_dir(self, override=None, subdir=None):
        ...
        if override:
            self.settings.flow['run_dir'] = Path(override)
    ...
    def copy_from_template(self, resource_name):
        ...
        rendered_content = template.render(flow=self.settings.flow, design=self.settings.design)
    ...
    def run(self, flow):
        ...
        if not self.source_paths_fixed:
            ...
            make_rel_paths('sources')
            make_rel_paths('tb_sources')
    ...
    def run_process(self, prog, prog_args, check=True):
        ...
        with open(stdout_logfile, 'w') as log_file:
            try:
                ...
                with subprocess.Popen([prog, *prog_args], cwd=self.run_dir,
                                      stdout=subprocess.PIPE,
                                      universal_newlines=True, encoding='utf-8', errors='replace') as proc:
                    while True:
        ...
        # proc = asyncio.run(self.run_process_async(prog, args))
    ...
    def find_fmax(self):
        ...
        while True:
            ...
            next_period = set_period - wns

            if best_period and not success:
                failed_runs += 1
    ...
    def parse_report(self, reportfile_path, re_pattern, *other_re_patterns):
        ...
        with open(reportfile_path) as rpt_file:
            ...
            def match_pattern(pat):
                match = re.search(pat, content, re.MULTILINE | re.DOTALL | re.IGNORECASE)
                if match is None:
                    return False
    ...
    def dump_results(self, flow):
        ...
        if results_json.exists():
            self.logger.warning(f"Results file alreaty exsits. Overwriting!")
